...
const LEVELS: Level[] = [
  ...
  },
  {
    ...
    description: "AI Guardian's regex patterns are active.",
    description_ja: "AI Guardianの正規表現パターンが有効です。",
    secret: "STARFALL",
...
export async function POST(request: NextRequest) {
  ...
  if (level.useInputFilter) {
    ...
    if (scan.score >= level.blockThreshold) {
      return NextResponse.json({
        response: `🛡️ AI Guardian blocked your request! (Risk score: ${scan.score}/100)\nDetected: ${scan.rules.map((r) => r.name).join(", ")}`,
        blocked: true,
  ...
  if (level.useSimilarity && checkSimilarity(prompt)) {
    return NextResponse.json({
      response: "🧠 AI Guardian's semantic similarity engine detected your intent!\nEven paraphrased attacks can't bypass Layer 2.",
      blocked: true,
