  ...
  return (
    <DocsLayout toc={TOC}>
      ...
      <section id="overview">
        <p>
          {ja
            ? "AI Guardianは、日本のAI関連規制・ガイドラインに対応した唯一の日本語ネイティブAIセキュリティプラットフォームです。マイナンバー検知、日本語プロンプトインジェクション対策、Human-in-the-Loop（人間承認フロー）により、AI推進法・AI事業者ガイドライン・個人情報保護法の要件を技術的にカバーします。"
      ...
      <section id="regulations">
        ...
        <p>
          {ja
            ? "日本初のAI法。事業者に「協力努力義務」を課し、透明性確保と人間の関与を重視。罰則はないが、調査協力義務と行政指導・名称公表の仕組みあり。AI Guardianの監査ログとHuman-in-the-Loopは、この法律の趣旨に直接対応します。"
        ...
        <p>
          {ja
            ? "マイナンバーは「特定個人情報」として法律で厳格に保護されており、漏洩は刑事罰の対象です。AI Guardianはマイナンバー（12桁）の入力・出力両方で検知し、自動サニタイズ機能で墨消しも可能です。"
      ...
      <section id="mapping">
        ...
        <p>{ja ? "AI Guardianの各機能が、どの日本の規制要件に対応しているかを示します。" : "Shows how each AI Guardian feature maps to specific Japanese regulatory requirements."}</p>

        <div className="not-prose overflow-x-auto my-4">
          <table className="min-w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
            <thead>
              <tr className="bg-gray-50">
                <th className="text-left px-4 py-2 font-semibold text-gray-700 border-b">{ja ? "AI Guardian機能" : "AI Guardian Feature"}</th>
      ...
      <section id="japanese-detection">
        ...
        <pre><code>{`from ai_guardian import scan, sanitize
...
`}</code></pre>
      ...
      <section id="industry">
        ...
        <p>
          {ja
            ? "銀行・証券・保険でのLLM活用において、口座情報・クレジットカード番号の漏洩防止、FISC安全対策基準への準拠が求められます。AI Guardianは入出力両方で金融PII検知を行い、コンプライアンスレポートで監査対応が可能です。"
        ...
        <p>
          {ja
            ? "患者情報・診療データのLLMへの送信は個人情報保護法上の重大リスクです。AI Guardianの自動サニタイズ機能で、患者情報を墨消ししてからLLMに送信できます。"
        ...
        <p>
          {ja
            ? "2026年4月から「行政AI調達ガイドライン」が全面適用されます。AI Guardianの監査ログ・Human-in-the-Loop・コンプライアンスレポートは、行政機関の調達要件を満たす基盤となります。"
      ...
      <section id="getting-started">
        ...
        <p>
          {ja
            ? "AI Guardianの導入は3ステップで完了します。日本語検知パターンは初期状態で有効です。"
