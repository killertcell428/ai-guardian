...
export default function QuickstartPage() {
  ...
  return (
    <DocsLayout toc={TOC}>
      ...
      <p className="lead">
        {ja
          ? "AI Guardian の導入は5分以内で完了します。既存の OpenAI クライアントの"
          : "Integrate AI Guardian in under 5 minutes. All you need to change is the"}{" "}
        <code>base_url</code>{" "}
      ...
      <ul>
        <li>{ja ? "AI Guardian アカウント（フリープランあり — カード不要）" : "An AI Guardian account (free tier available — no credit card required)"}</li>
        <li>{ja ? "AI Guardian APIキー（" : "An AI Guardian API key (starts with "}<code>aig_</code>{ja ? " で始まる）" : ")"}</li>
      ...
      <p>
        {ja
          ? "サインアップ後、AI Guardianダッシュボードの"
          : "After signing up, go to"}{" "}
        <strong>Settings → API Keys</strong>{" "}
        {ja
          ? "で新しいキーを作成します。キーは"
          : "in the AI Guardian dashboard and create a new key. Copy the key — it starts with"}{" "}
      ...
      <p>
        ...
        <code>base_url</code>{" "}
        {ja
          ? "を AI Guardian のエンドポイントに書き換えます。それ以外は何も変える必要はありません。"
          : "with your AI Guardian endpoint. Everything else stays the same."}
      </p>
      ...
      <h2 id="step5">{ja ? "5. AI Guardian のレスポンスを処理する" : "5. Handle AI Guardian Responses"}</h2>
      <p>
        {ja
          ? "AI Guardian は3種類の結果を返します。コードで3つすべてを処理してください："
