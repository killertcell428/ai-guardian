# AI Guardian — Automated Development Loop Design

> Last updated: 2026-04-03
> Overview: A semi-automated loop of Security Research -> Feature Development -> Article Publishing to continuously strengthen AI Guardian
...
- **Summary**: ...
- **Source**: URL
- **AI Guardian Relevance**: High/Medium/Low
- **Mitigation Direction**: What ai-guardian can address
...
### Purpose
Read findings from the Research Scout, identify gaps with ai-guardian's current state, and implement necessary features.

### Process
1. **Gap Analysis**: Read entries with `status: new` from `content/research_backlog/`
2. **Prioritization**: Prioritize by AI Guardian relevance x implementation cost
3. **Implementation**: Execute one of the following:
   - Add new detection patterns to `ai_guardian/patterns.py`
   - Add Guard class patterns to `ai_guardian/filters/patterns.py`
   - Add new attack phrases to `ai_guardian/similarity.py`
   - Add new policy templates to `policy_templates/`
   - Add new regulatory mappings to `ai_guardian/compliance.py`
...
[General countermeasures + how AI Guardian addresses them]

## Implementation with AI Guardian
...
### Article Guidelines
- **Value first**: 80% of the article should be pure technical content. AI Guardian promotion should be naturally woven in and limited to 20% or less
