...
def _build_compliance_items() -> list[ComplianceItem]:
    return [
        ...
        ComplianceItem(
            ...
            ai_guardian_feature="監査ログでリクエスト単位の判定履歴を記録。レポートで期間別集計を出力。",
            status="partial",
            notes="AIモデル自体の更新履歴管理は対象外（LLMプロバイダーの責任）。AI Guardianはフィルター判定のログを管理。",
        ),
        ComplianceItem(
            regulation="AI事業者ガイドライン v1.1",
        ...
        ComplianceItem(
            ...
            ai_guardian_feature="scan_rag_context()でRAG検索結果のスキャン。ドキュメントチャンク単位での検知。",
            status="partial",
            notes="データリネージ自体の管理機能はない。入力データのスキャンのみ。",
        ),
        ComplianceItem(
            regulation="AI事業者ガイドライン v1.1",
        ...
        ComplianceItem(
            ...
            ai_guardian_feature="AI Guardianはプロキシとして入力を検査するが、タグ付け自体はLLMプロバイダー/アプリ側の責任。",
            status="partial",
            notes="入力のスキャンは行うが、プロンプト構造化（XMLタグ等での分離）はアプリケーション側で実装が必要。ドキュメントで推奨事項を提供。",
        ),
        ComplianceItem(
            regulation="AIセキュリティ技術ガイドライン（総務省）",
        ...
        ComplianceItem(
            ...
            ai_guardian_feature="AI Guardianはプロキシとして読み取り専用で動作。LLMへのアクセス制御はポリシーエンジンで管理。",
            status="partial",
            notes="AI Guardian自体は最小権限で動作するが、接続先LLMの権限管理はユーザーの責任。",
        ),
        ...
        ComplianceItem(
            ...
            ai_guardian_feature="著作物全文コピー要求の検知パターン。修復ヒントで出典明記と要約利用を推奨。",
            status="partial",
            notes="著作権侵害の判断は法的判断であり、自動化には限界がある。パターンマッチングで明らかなリスクのみ検知。",
        ),
    ]
