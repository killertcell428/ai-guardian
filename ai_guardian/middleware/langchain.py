"""LangChain callback handler for ai-guardian.
...
try:
    ...
    raise ImportError(
        ...
        "Install it with: pip install 'ai-guardian[langchain]'"
    ) from e


class GuardianBlockedError(RuntimeError):
    """Raised when AI Guardian blocks an LLM input or output."""
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        for prompt in prompts:
            ...
            if result.blocked:
                self._handle_block(
                    f"AI Guardian blocked LLM input (score={result.risk_score})",
                    result.risk_score,
                    result.reasons,
                )
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        for message_group in messages:
            ...
            if result.blocked:
                self._handle_block(
                    f"AI Guardian blocked chat input (score={result.risk_score})",
                    result.risk_score,
                    result.reasons,
                )
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        for generations in response.generations:
            for gen in generations:
                ...
                if result.blocked:
                    self._handle_block(
                        f"AI Guardian blocked LLM output (score={result.risk_score})",
                        result.risk_score,
                        result.reasons,
                    )
