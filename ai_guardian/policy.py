...
def load_policy(path: str = "ai-guardian-policy.yaml") -> Policy:
...
def _matches(event: "ActivityEvent", rule: PolicyRule) -> bool:
    ...
    if rule.target != "*" and event.target:
        ...
        if not fnmatch(target, pattern) and not fnmatch(
...
def _default_policy() -> Policy:
    ...
    return Policy(
        name="AI Guardian Default Policy",
        version="1.0",
        default_decision="allow",
        rules=[
            # === Dangerous commands ===
            ...
            PolicyRule(
                ...
                target="*",
                decision="allow",
                reason="LLM prompts are scanned by AI Guardian's detection engine",
            ),
        ],
    )


def save_policy(policy: Policy, path: str = "ai-guardian-policy.yaml") -> None:
    """Save a policy as a YAML-like file (readable without PyYAML)."""
    lines = [
        f"# AI Guardian Policy: {policy.name}",
        f"# Generated by AI Guardian v0.3.0",
        f"",
        f"name: \"{policy.name}\"",
