...
AI Guardian v1.1.0では、MCPサーバー全体を1コマンドで評価できる`scan_mcp_server()`を実装した。
...
**AI Guardianの立ち位置**: MCPだけでなく、入力/出力/RAG/会話履歴を含む**統合セキュリティレイヤー**。MCPスキャンは機能の一部。他のツールはMCPスキャン専門。

**逆に、AI Guardianに足りないもの**:
...
AI Guardian v1.1.0はこの3つをゼロ依存・~50usレイテンシで実現する。
