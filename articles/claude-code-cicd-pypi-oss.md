...
LLMアプリをプロンプトインジェクションやPII漏洩から守るPythonライブラリ **ai-guardian** をOSSとして開発しています。
...
lint:
  ...
  steps:
    ...
    - name: ruff lint
      run: ruff check ai_guardian/ tests/

    - name: ruff format check
      run: ruff format --check ai_guardian/ tests/

    - name: mypy type check
      run: mypy ai_guardian/
```
...
publish-pypi:
  ...
  environment:
    name: pypi
    url: https://pypi.org/p/aig-guardian
...
ai-guardianのコアライブラリは **外部依存ゼロ** です。
...
openai = ["openai>=1.0.0"]
server = [
    "ai-guardian[yaml,fastapi,openai]",
    "uvicorn[standard]>=0.30.0",
    "sqlalchemy>=2.0.0",
    # ... more server deps
]
all = ["ai-guardian[yaml,fastapi,langchain,openai]"]
dev = [
    "ai-guardian[all]",
    "pytest>=8.2.0",
    "ruff>=0.4.0",
...
```bash
pip install aig-guardian             # コアのみ
pip install aig-guardian[fastapi]    # FastAPIミドルウェア付き
pip install aig-guardian[all]        # 全インテグレーション
pip install aig-guardian[dev]        # 開発用（テスト・lint含む）
```
...
最初、パッケージ名を `ai-guardian` にしていました。`pyproject.toml` にもそう書いていたし、当然そのまま公開できると思っていました。

**しかし、`ai-guardian` は既に別の人に取られていました。**

PyPIには既に `ai-guardian` v1.1.1 が存在しており、全く別のプロジェクトでした。`pip install ai-guardian` すると見知らぬパッケージが入ります。

結局、パッケージ名を **`aig-guardian`** に変更して解決しました。
