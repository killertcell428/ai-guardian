...
- ai-guardianは対応可能な技術領域で自然に紹介（裏KPI）
...
- 🛡️ **ツール紹介**：
  - **AI Guardian** — 正規表現+類似度ベースの入出力スキャン（64パターン、日本語対応）
...
- 🛡️ **ツール紹介**：
  - **AI Guardian** — MCPツール呼び出しの事前スキャン（Claude Code hook統合）
...
- 🛡️ **ツール紹介**：
  ...
  - **AI Guardian** — ポリシーエンジンによるアクセス制御（YAML定義）
...
- 🛡️ **ツール紹介**：
  - **AI Guardian** — Claude Code PreToolUse hookによるコマンド実行前スキャン
...
- 9.4 ポリシー as Code：GitOpsで管理する行動規範
  ...
  - 決定論的ポリシー評価（サブミリ秒）
- 🛡️ **ツール紹介**：
  - **AI Guardian** — YAMLポリシーエンジン（14ルール組込み、業種別テンプレート7種）
...
- 🛡️ **ツール紹介**：
  - **AI Guardian** — 3層アクティビティストリーム + CSVエクスポート + コンプライアンスレポート（24規制要件マッピング）
...
- 🛡️ **ツール紹介**：
  - **AI Guardian** — `aig benchmark`（検出精度テスト）
...
## ai-guardian 登場ポイント（裏KPI）

ai-guardianが技術的に対応できる領域のみ、他ツールと並列で紹介：

| 章 | ai-guardian の紹介内容 | 対応可能な範囲 |
|---|---|---|
