...
Processes player attempts through AI Guardian's own detection engine,
...
def process_attempt(
    ...
    if level.use_similarity:
        ...
        if sim_matches:
            ...
            return _build_response(
                response=(
                    f"🧠 AI Guardian's semantic similarity engine detected your intent!\n\n"
...
def _blocked_message(risk_score: int, reason: str, level: LevelConfig) -> str:
    return (
        f"🛡️ AI Guardian blocked your request! (Risk score: {risk_score}/100)\n\n"
        f"Detected: {reason}\n\n"
        f"Your prompt triggered AI Guardian's security filters. "
