# AI Guardian VS Code Extension — Architecture Design

## Overview

The VS Code extension provides developer-facing tooling for the `ai-guardian` Python library.
...
1. **Lightweight host process** — The extension contains zero Python and zero LLM inference.
   All detection logic lives in the installed `ai_guardian` Python library and is invoked via
   `aig scan --json` subprocesses.

2. **Graceful degradation** — If Python or `ai_guardian` is not found the extension disables
...
│       │                                                              │
│       ├── GuardianService  ──spawn──► python -m ai_guardian.cli scan │
│       │        │                         --json  (stdin/stdout)      │
│       │        │                     └── ScanResult JSON             │
│       │        │                                                      │
│       ├── DiagnosticProvider                                         │
...
```
python -m ai_guardian.cli scan --json
```
...
written to the "AI Guardian" output channel; it never surfaces as a UI error on its own.
...
|--------------------------------|-------------|---------------------------------------------------------------|
| `aiGuardian.pythonPath`        | `"python"`  | Python executable with `ai_guardian` installed                |
...
|---------------------------------|--------------------------------|--------------|
| `aiGuardian.scanSelection`      | Scan with AI Guardian          | Yes (editor) |
| `aiGuardian.scanFile`           | Scan File with AI Guardian     | Yes (editor) |
| `aiGuardian.clearDiagnostics`   | Clear AI Guardian Diagnostics  | No           |
| `aiGuardian.showOutput`         | Show AI Guardian Output        | No           |
| `aiGuardian.openSettings`       | Open AI Guardian Settings      | No           |
...
2. **Project Policy** (`aiGuardian.policyView`) — TreeView showing the active
   `ai-guardian-policy.yaml` metadata (name, version, rule count, default decision).
...
| `ai_guardian` not installed | Single notification at activation; scanning silently no-ops  |
...
- **Remote scan mode** — call `ai_guardian.server` HTTP API instead of spawning a process.
