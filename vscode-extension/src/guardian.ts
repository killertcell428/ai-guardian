...
export class PythonNotFoundError extends Error {
  constructor(pythonPath: string) {
    super(
      `AI Guardian: could not launch Python executable "${pythonPath}". ` +
        `Check the aiGuardian.pythonPath setting and ensure ai_guardian is installed ` +
        `in that environment (pip install ai-guardian).`
    );
...
export class ScanTimeoutError extends Error {
  constructor(text: string) {
    super(
      `AI Guardian: scan timed out after ${SCAN_TIMEOUT_MS}ms ` +
...
export class GuardianService implements vscode.Disposable {
  ...
  async scan(text: string, config: GuardianConfig): Promise<ScanResult> {
    return new Promise<ScanResult>((resolve, reject) => {
      ...
      const args = ["-m", "ai_guardian.cli", "scan", "--json"];
      ...
      child.on("close", (code) => {
        settle(() => {
          ...
          try {
            ...
            reject(new Error(`AI Guardian: failed to parse scan output: ${parseErr}`));
          }
        });
      });
  ...
  /**
   * Verify that the configured Python has ai_guardian installed.
  ...
  async checkInstallation(pythonPath: string): Promise<string> {
    return new Promise((resolve) => {
      const child = cp.spawn(
        pythonPath,
        ["-c", "import ai_guardian; print(ai_guardian.__version__)"],
      ...
      child.on("close", (code) => {
        if (code === 0) {
          resolve(`ai_guardian ${out.trim()} — ready`);
        } else {
          resolve(
            `ai_guardian not installed (exit ${code}). Run: ${pythonPath} -m pip install ai-guardian\n${err.trim()}`
          );
        }
      });
    });
