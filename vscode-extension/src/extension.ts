/**
 * AI Guardian VS Code Extension — main entry point.
 *
 * Lifecycle:
...
export function activate(context: vscode.ExtensionContext): void {
  const outputChannel = vscode.window.createOutputChannel("AI Guardian");
  outputChannel.appendLine("AI Guardian extension activated.");
  ...
  const scanSelectionCmd = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      if (!editor) {
        vscode.window.showWarningMessage("AI Guardian: no active editor.");
        return;
      }
      if (editor.selection.isEmpty) {
        vscode.window.showWarningMessage(
          "AI Guardian: select some text first, then run Scan with AI Guardian."
        );
        return;
      }
      ...
      try {
        ...
        if (result.risk_level === "HIGH" || result.risk_level === "CRITICAL") {
          const msg =
            `AI Guardian: ${result.risk_level} threat detected ` +
        ...
        if (err instanceof PythonNotFoundError) {
          ...
          vscode.window.showErrorMessage(`AI Guardian error: ${err}`);
  ...
  const scanFileCmd = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      if (!SUPPORTED_LANGUAGES.has(editor.document.languageId)) {
        vscode.window.showWarningMessage(
          `AI Guardian: file type "${editor.document.languageId}" is not supported.`
        );
        return;
      }
      ...
      await vscode.window.withProgress(
        {
          ...
          title: "AI Guardian: scanning file…",
          cancellable: false,
        },
        async () => {
      ...
      if (errors + warnings === 0) {
        vscode.window.showInformationMessage(
          "AI Guardian: no threats detected in this file."
        );
      } else {
        vscode.window.showWarningMessage(
          `AI Guardian: found ${errors} error(s) and ${warnings} warning(s). ` +
  ...
  // Command: clear all AI Guardian diagnostics.
  ...
  const openSettingsCmd = vscode.commands.registerCommand(
    ...
    () => {
      vscode.commands.executeCommand(
        "workbench.action.openSettings",
        "@ext:ai-guardian.vscode-ai-guardian"
      );
    }
  );
...
async function loadPolicyStatus(
  ...
  return new Promise((resolve) => {
    ...
    const script = [
      ...
      "    p = load_policy('ai-guardian-policy.yaml')",
