...
const DIAGNOSTIC_SOURCE = "AI Guardian";
...
export class DiagnosticProvider implements vscode.Disposable {
  ...
  async scanDocument(document: vscode.TextDocument): Promise<void> {
    ...
    for (const { value, startIndex } of literals) {
      ...
      try {
        ...
        console.error("[AI Guardian] scan error:", err);
        continue;
      }
  ...
  async scanSelection(
    ...
    try {
      ...
      if (err instanceof PythonNotFoundError) {
        ...
        vscode.window.showErrorMessage(`AI Guardian scan failed: ${err}`);
      }
      return null;
    }
  }
  ...
  private _buildDiagnostic(
    ...
    const message =
      `AI Guardian: ${ruleLabel}${owaspRef} ` +
