...
## 解決策: AI Guardian — `pip install` して `aig init` するだけ

[AI Guardian](https://github.com/cto-and-sec/ai-guardian) は、AIエージェントのセキュリティとガバナンスを担うPythonライブラリだ。
...
`aig init --agent claude-code` を実行すると、`.claude/hooks/` にインターセプトスクリプトが生成される。以後、Claude Codeの**すべてのツール呼び出し**（Bash, Read, Write, Edit, WebFetch...）がAI Guardianを通過する。

```
ユーザー → Claude Code → [AI Guardian Hook] → ツール実行
                              ↓
                         脅威スキャン
                         ポリシー評価
                         ログ記録
...
AI Guardianは53パターンの脅威検出を内蔵している。試しに叩いてみよう。
...
`.ai-guardian/policy.yaml` にルールを書く：
...
| 層 | 場所 | 用途 |
|----|------|------|
| ローカル | `.ai-guardian/logs/` | 開発者が自分の操作を確認 |
| グローバル | `~/.ai-guardian/global/` | CISO・管理者がチーム全体を監査 |
| アラート | `~/.ai-guardian/alerts/` | ブロック・レビューイベントの永続保存 |
...
AI Guardianのコア部分は**Python標準ライブラリだけ**で動く。`pip install aig-guardian` で余計なものが入らない。
...
```bash
$ aig benchmark
AI Guardian Benchmark Suite v0.6.1
═══════════════════════════════════
...
AI Guardianは Claude Code 専用ではない。以下のフレームワークにも対応している：

```python
# FastAPIミドルウェアとして
from ai_guardian.middleware import AIGuardianMiddleware
app.add_middleware(AIGuardianMiddleware)

# LangGraphのノードとして
from ai_guardian.middleware.langgraph import GuardNode
...
筆者は AI Guardian の開発自体を Claude Code の Cowork モードで行い、Scheduled Tasks で定期的なベンチマーク実行やログローテーションを自動化している。
...
AI Guardianは24の国内規制要件をマッピングしている：
...
| **プロンプト/入力** | **プロンプトインジェクション** | **AI Guardian** |
| **出力** | **PII漏洩、秘密鍵流出** | **AI Guardian** |
| **操作ログ** | **監査証跡** | **AI Guardian** |

サンドボックスは「OSレベルの隔離」、AI Guardianは「プロンプト/データレベルの防御」。**両方使うのが正解**だ。
...
**GitHub**: [cto-and-sec/ai-guardian](https://github.com/cto-and-sec/ai-guardian)
**PyPI**: [aig-guardian](https://pypi.org/project/aig-guardian/)
