...
## The Fix: 3 Lines with AI Guardian
...
```python
from ai_guardian.middleware.langchain import GuardianBlockedError
...
| Scenario | Without AI Guardian | With AI Guardian |
|----------|-------------------|-----------------|
...
If you are using LangGraph for stateful, multi-step agent workflows, AI Guardian provides a `GuardNode` that drops into your `StateGraph` as a first-class node:

```python
from langgraph.graph import StateGraph, END
from ai_guardian.middleware.langgraph import GuardNode, GuardState
...
```python
from ai_guardian.middleware.langgraph import GuardianBlockedError
...
```python
from ai_guardian.middleware.langgraph import GuardNode, GuardState, GUARD_BLOCKED
...
AI Guardian includes dedicated MCP tool scanning:
...
from ai_guardian import scan_mcp_tool
...
AI Guardian includes a built-in red team suite that generates adversarial attacks across 9 categories and runs them against your detection pipeline:

```python
from ai_guardian.redteam import RedTeamSuite

# Test AI Guardian's own detection
suite = RedTeamSuite()
results = suite.run()
...
AI Guardian ships with a benchmark suite containing known attack samples and verified safe inputs to measure precision and recall:
...
Sometimes you want to process the input instead of blocking it. AI Guardian can auto-redact PII before it reaches the LLM:
...
AI Guardian is open source under the MIT license. If you are building LangChain apps that handle real user input, give it a try and let me know what you think.

**GitHub**: [https://github.com/chuuuuuuuuuu/ai-guardian](https://github.com/chuuuuuuuuuu/ai-guardian)
