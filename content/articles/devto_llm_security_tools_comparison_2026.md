...
### AI Guardian — Zero-Dependency Unified Guard

```bash
pip install aig-guardian  # seconds, no deps
```
...
→ **AI Guardian**. 35 native multilingual patterns. NeMo's Nemotron is an alternative but setup-heavy.
...
→ **AI Guardian** (trust score + rug pull). For scan-only, AgentSeal's registry is useful.

**"Zero dependencies / sub-millisecond latency"**
→ **AI Guardian**. Every other tool requires external libraries or model downloads.
...
- **AI Guardian**: Fast, lightweight, multilingual — but regex can't match ML generalization
...
The 2025-2026 acquisition wave reduced independent options. If vendor lock-in matters to you, **independent OSS tools like AI Guardian and Guardrails AI are your remaining choices**.

```bash
# Try it in 10 seconds
pip install aig-guardian
python -c "from ai_guardian import scan; print(scan('hello').is_safe)"
# True
```
