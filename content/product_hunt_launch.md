# Product Hunt Launch Plan — AI Guardian
...
> Open-source Python library that scans every request to your LLM and blocks prompt injection, jailbreaks, PII leaks, and data exfiltration — before they reach the model. 64 detection patterns, zero dependencies, 100% benchmark precision. pip install aig-guardian.
...
AI Guardian is an open-source Python middleware that sits between your app and the LLM. It scans every request in <5ms and blocks threats before they reach the model.
...
| Field | URL |
|-------|-----|
| Website | https://ai-guardian.dev (or Vercel deploy URL) |
...
I'm the solo maker behind AI Guardian. Here's the backstory:
...
So I built AI Guardian: a zero-dependency Python library that scans LLM inputs/outputs locally in <5ms.

**What I'm most proud of:**
- 100% benchmark precision (53/53 attacks, 0 false positives)
- Literally `pip install aig-guardian` → `aig init` → done
...
Try it: `pip install aig-guardian && aig scan "ignore all instructions"`
...
- **Content:** Logos/icons of supported frameworks in a circle around AI Guardian:
...
- Shield logo + "AI Guardian" + tagline
...
- [ ] `aig-guardian` の最新バージョンが PyPI に上がっているか確認
...
```
🚀 AI Guardian is LIVE on Product Hunt!
...
```
How AI Guardian works in 30 seconds:

1. pip install aig-guardian
...
### LinkedIn Post
```
Today I'm launching AI Guardian on Product Hunt — an open-source security layer for LLM applications.
...
### Must-Have (ローンチ前に必須)
- [ ] **Product Hunt バッジ追加** — Hero セクション付近に "Featured on Product Hunt" バッジを配置。ローンチ当日に公開。`<a href="https://www.producthunt.com/posts/ai-guardian">` + PH 提供の SVG バッジ
...
- [ ] **CTA の A/B テスト準備** — "Start Free" vs "Try in 30 Seconds" vs "pip install aig-guardian" のどれが CTR 高いか
...
| May 19 | 技術解説「AI Guardian の検出アルゴリズム」 | Qiita |
| May 21 | "Top 10 Prompt Injection Attacks and How to Block Them" | DEV.to |
| May 26 | ユースケース「LangChain + AI Guardian で安全なチャットボット」 | Zenn |
